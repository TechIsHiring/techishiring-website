...
declare global {
  namespace Cypress {
    interface Chainable {
        validateLink(name: string, destination : string, ancestorSelector: string, target?: 'new tab'): Chainable<void>
    }
  }
}
   ...
   * @param ancestorSelector - a specific part of the DOM to test
   * @param target - target attribute - e.g. "new tab" or not
   */
  const validateLink = (name: string, destination : string, ancestorSelector: string, target?: 'new tab') => {
    ...
    cy.contains(`${ancestorSelector} a`, name)
